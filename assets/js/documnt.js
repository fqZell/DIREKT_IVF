...
const accordionDocumnt = () => {
    ...
    if (!accordionImg) return

    const accordionContent = document.querySelectorAll(".document-wrapper__content")
    if (!accordionContent) return

    accordionLink.forEach((item, index) => {
        item.addEventListener("click", () => {
            accordionContent[index].classList.toggle("active")
            accordionImg[index].classList.toggle("active")
        })
    })
}